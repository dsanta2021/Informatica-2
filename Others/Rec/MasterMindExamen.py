import random
import os           #Para borrar ficheros(Examen)
...
class MasterMindGame:
    ...
    def newTurn(self, combiPlay: str):
        ...
        if lSecretCode == eMatches:
            ...
            for names in self.mmTopPlayers.keys():
                if names == name:
                    name = name + '1'
            print('Su nombre está cogido. Su nombre será: ', name)

            self.updateTopPlayers(name)
...
########  EXAMEN  #######
    def readTopPlayers(self):
        ...
        for player in players:
            match = player.split('-')
            #print(match)
            self.mmTopPlayers[match[0]] = match[1]

        f.close()


    def updateTopPlayers(self, playerName: str):
        mayorTurn = 0
        mayorName = ''

        for name, turns in self.mmTopPlayers.items():
            if int(turns) >= mayorTurn:
                mayorTurn = int(turns)
                mayorName = name
        print(self.currentTurn)
        if self.currentTurn <= mayorTurn:
            self.mmTopPlayers.pop(mayorName)
            self.mmTopPlayers[playerName] = self.currentTurn
            print('Comprueba si eres un Top 3 MasterMind')
            for name, turns in self.mmTopPlayers.items():
                print('MVP ', name, ' acertó la clave en ', turns, 'intentos.')
    ...
    def writeTopPlayers(self):
        ...
        for name, turns in self.mmTopPlayers.items():
            allJoin = allJoin + str(name) + '-' + str(turns) + '#'

        long = len(allJoin) - 1
        allJoin = allJoin[0:long]
        print(allJoin)

        os.remove('topMMPlayers.txt')
        newF = open('topMMPlayers.txt', 'wt')
        newF.write(allJoin)
        newF.close()

        nF = open('topMMPlayers.txt', 'rt')
        print(nF.read())
        nF.close()



mm = MasterMindGame('rrrr')
