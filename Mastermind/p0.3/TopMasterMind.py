...
class MasterMindGame:
    ...
    def newTurn(self, combiPlay: str):
        ...
        print(f'Va por el turno: {self.currentTurn}. Le quedan {self.maxTurns-self.currentTurn} turnos.') #Lo he añadido poruqe creo que es importante saber en que turno estás
    ...
    def readTopPlayers():
        mmTopPlayers = {}
        file = open("topMMPlayers.txt", "r")
        readfile = file.read()
        #print(readfile)
        separar = readfile.split('#')
        #print(separar)
        jugador1 = separar[0]
        jugador2 = separar[1]
        jugador3 = separar[2]
        #print(jugador3)
        #print(separar[1].split('-'))
        allSeparado = list(separar[0].split('-') + separar[1].split('-') + separar[2].split('-'))
        #print(allSeparado)
        mmTopPlayers[allSeparado[0]] = allSeparado[1]
        mmTopPlayers[allSeparado[2]] = allSeparado[3]
        mmTopPlayers[allSeparado[4]] = allSeparado[5]
        for (name, tirada) in mmTopPlayers.items():
            print(f'Nombre: {name} --> Ganó en la ronda {tirada}')

    def updateTopPlayers (self, name, numTirada):
        # Primero pedir al jugador su nombre y número de tiradas con las que ha conseguido acertar
        name = input()
        numTirada = input()
        print('Escriba su nombre:', name)
        print(f'Ha acertado con {numTirada} la combinación')
        #for name, numTirada in s.items():
            #if numTirada >= 9:  # si el número de tirada es menor que el más pequeños del diccionario


MasterMindGame.readTopPlayers()
#MasterMindGame.updateTopPlayers()